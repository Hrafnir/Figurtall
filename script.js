...
const app = {
    canvas: null, ctx: null, marquee: null,
    n: 1, shapes: [], nextId: 1, clipboard: [],
    ...
    handleContextMenu(e) {
        ...
        for (let i = this.shapes.length - 1; i >= 0; i--) {
            if (this.shapes[i].hitTest(this.n, pos.gridX, pos.gridY)) { hitShape = this.shapes[i]; break; }
        }
        if (hitShape) {
    ...
    handleMouseDown(e) {
        ...
        if (hitShape) {
            ...
            if (e.shiftKey) {
                if (alreadySelected) this.selectedIDs.delete(hitShape.id); else this.selectedIDs.add(hitShape.id);
            } else {
                if (!alreadySelected) { this.selectedIDs.clear(); this.selectedIDs.add(hitShape.id); }
            }
            if (!e.shiftKey && hitShape.groupName) {
                this.shapes.forEach(s => { if (s.groupName === hitShape.groupName) this.selectedIDs.add(s.id); });
            }
            this.isDragging = true; this.dragStart = { x: pos.gridX, y: pos.gridY };
            this.updateUI(); 
        } else {
            if (!e.shiftKey) { this.selectedIDs.clear(); this.updateUI(); }
            this.isBoxSelecting = true; this.boxStart = { x: pos.x, y: pos.y }; 
            this.marquee.style.display = 'block';
            this.marquee.style.left = pos.x + 'px'; this.marquee.style.top = pos.y + 'px';
            this.marquee.style.width = '0px'; this.marquee.style.height = '0px';
        }
        this.draw();
    },
    handleMouseMove(e) {
        ...
        if (this.isDragging) {
            ...
            const x = Math.min(this.boxStart.x, pos.x); const y = Math.min(this.boxStart.y, pos.y);
            const w = Math.abs(pos.x - this.boxStart.x); const h = Math.abs(pos.y - this.boxStart.y);
            this.marquee.style.left = x + 'px'; this.marquee.style.top = y + 'px';
            this.marquee.style.width = w + 'px'; this.marquee.style.height = h + 'px';
        } else {
    ...
    handleMouseUp(e) {
        ...
        if (this.isBoxSelecting) {
            this.isBoxSelecting = false; this.marquee.style.display = 'none';
            const pos = this.getGridPos(e);
            const bX1 = Math.min(this.boxStart.x, pos.x); const bX2 = Math.max(this.boxStart.x, pos.x);
            const bY1 = Math.min(this.boxStart.y, pos.y); const bY2 = Math.max(this.boxStart.y, pos.y);
            this.shapes.forEach(s => {
    ...
    deleteSelected() {
        ...
        this.selectedIDs.clear(); this.updateUI(); this.draw();
    },
    groupSelected() {
        ...
        [...this.shapes].reverse().forEach(shape => {
            ...
            controls.innerHTML = `
                <div class="col-span-2 grid grid-cols-2 gap-2 mb-1">
                    <div><label class="text-gray-400">Pos X</label><input type="text" value="${shape.posX}" class="w-full border rounded px-1" onchange="app.updateSelectedProp('posX', this.value)"></div>
                    <div><label class="text-gray-400">Pos Y</label><input type="text" value="${shape.posY}" class="w-full border rounded px-1" onchange="app.updateSelectedProp('posY', this.value)"></div>
                </div>
                <div><label class="text-gray-400">Rotasjon</label><div class="flex items-center gap-1"><input type="range" min="0" max="360" step="5" value="${shape.rotation}" class="w-full" oninput="app.shapes.find(s=>s.id===${shape.id}).rotation=parseInt(this.value); this.nextElementSibling.value=this.value; app.draw();"><input type="number" class="w-10 border rounded px-1 text-center" value="${shape.rotation}" onchange="app.shapes.find(s=>s.id===${shape.id}).rotation=parseFloat(this.value); this.previousElementSibling.value=this.value; app.draw();"></div></div>
                <div><label class="text-gray-400">Offset ($n$)</label><input type="number" value="${shape.nOffset}" class="w-full border rounded px-1" onchange="app.shapes.find(s=>s.id===${shape.id}).nOffset=parseInt(this.value); app.updateFormula(); app.draw();"></div>
            `;
